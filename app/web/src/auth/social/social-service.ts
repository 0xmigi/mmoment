...
export class SocialService {
  ...
  private readonly API_BASE = `${window.location.protocol}//${window.location.hostname}/api`;
  ...
  async getProfilesByAddress(address: string): Promise<SocialProfile[]> {
    try {
      ...
      const response = await axios.get(`${this.API_BASE}/profiles/${address}`);
      ...
      try {
        ...
        console.error('Failed to get Farcaster profile directly:', err);
      }
      
      return [];
    } catch (error) {
      console.error('Failed to fetch profiles for address:', error);
      return [];
    }
  }
