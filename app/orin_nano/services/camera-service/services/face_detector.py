...
class FaceDetector:
    ...
    def __init__(self):
        ...
        if MTCNN_AVAILABLE:
            try:
                # Try different MTCNN initialization patterns for compatibility
                try:
                    # Try new MTCNN API first
                    self._detector = MTCNN(min_face_size=60, scale_factor=0.709)
                    self._detection_method = "mtcnn"
                    logger.info("Using MTCNN for face detection (new API)")
                except TypeError:
                    # Try old MTCNN API without min_face_size parameter
                    self._detector = MTCNN()
                    self._detection_method = "mtcnn"
                    logger.info("Using MTCNN for face detection (legacy API)")
            except Exception as e:
                logger.error(f"Error initializing MTCNN: {e}")
                self._detector = None
                MTCNN_AVAILABLE = False  # Disable MTCNN to prevent future attempts
