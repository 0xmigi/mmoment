...
class FaceService:
    ...
    def __init__(self):
        ...
        self._detection_interval = 0.5   # Reduced from 0.1 to 0.5 (2fps instead of 10fps)
        self._recognition_interval = 2.0  # Reduced from 1.0 to 2.0 (0.5fps instead of 1fps)
    ...
    def _processing_loop(self) -> None:
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if frame is None:
                    time.sleep(0.05)  # Longer sleep if no frame available
                    continue
                ...
                time.sleep(0.5)  # Increased from 0.2 to 0.5 (2Hz instead of 5Hz)
    ...
    def get_processed_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        Only creates a copy if visualization is actually needed.
        """
        ...
        if not self._boxes_enabled:
            return frame  # No visualization needed, return original
            
        try:
            # Get current detection and recognition results
            with self._results_lock:
                detected_faces = self._detected_faces.copy() if self._detected_faces else []
                recognized_faces = self._recognized_faces.copy() if self._recognized_faces else {}
            
            # If no faces detected, return original frame
            if not detected_faces and not recognized_faces:
                return frame
            
            # Create a copy only when we actually need to draw
            output = frame.copy()
